...
const taskDefinition = new aws.ecs.TaskDefinition(`${appName}-task`, {
  ...
  containerDefinitions: pulumi
    .all([ecrRepo.repositoryUrl, logGroupName, region, dbEndpoint, dbPasswordSecretArn, jwtSecret])
    .apply(([repoUrl, logGroup, awsRegion, dbHost, dbSecretArn, jwt]) => {
      const env = [...containerEnv];
      const secrets: { name: string; valueFrom: string }[] = [];

      env.push({ name: "JWT_SECRET", value: jwt });
